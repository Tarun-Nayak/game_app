...
const tts = document.querySelector(".tap_to_start");
const combo_span = document.querySelector("#combo_span");
const bg_music = document.querySelector("#bg_music");
bg_music.volume = 0.25;
const gem_collect =
    "http://contentservice.mc.reyrey.net/audio_v1.0.0/?id=73702d54-159c-5403-9706-99824dee8d9e";
...
var best_combo = 0;
var combo = [];
let pad_l = window.innerWidth * 0.45;
var kd = false;
var btn_speed = 1;
var frames = 60;
...
function addBlocks() {
    for (var i = 0; i < blocks; i++) {
        ...
        b.style.backgroundColor =
            "hsla(240deg,100%," + (Math.floor(i / blocksPer) * 10 + 50) + "%, .8)";
        b.style.left =
            (i % blocksPer) * (window.innerWidth * (100 / blocksPer / 100)) + "px";
        b.style.top =
            Math.floor(i / blocksPer) * (window.innerHeight * 0.08) + "px";
...
var ball_t = -1,
    ball_l = Math.random() > 0.5 ? 1 : -1,
    ball_speed = 5;

function playTheGame() {
    // initial game reset stuff
    bg_music.play();
    tts.removeEventListener("click", playTheGame);
    misses = 1;
    score = 0;
    round = 1;
    best_combo = 0;
    ...
    function moveBall() {
        ...
        // collison detection
        var half_ball = ball_loc.width * 0.5;
        if (ball_t > 0) {
            var b_class = document.elementFromPoint(
                ball_loc.left + half_ball,
                ball_loc.top + ball_loc.width
            );
        } else {
            var b_class = document.elementFromPoint(
                ball_loc.left + half_ball,
        ...
        if (b_class.classList.contains("block")) {
            ...
            if (!document.querySelector(".muted")) {
                ...
                got_gem.volume = Math.random() < 0.5 ? 1 : 0.5;
                got_gem.play();
            }
        ...
        if (misses > 3) {
            ...
            game_blocks.innerHTML =
                round * blocks - document.querySelectorAll(".block").length;
            game_combo.innerHTML = best_combo;
            game_score.innerHTML = score;

            tts.addEventListener("click", playTheGame);
    ...
    ball.style.top = window.innerHeight * 0.75 + "px";
    // moveBall()
...
function comboScore() {
    ...
    var sum = combo.reduce(function(a, b) {
        return a + b;
    }, 0);
    score += length * sum;
    document.documentElement.style.setProperty("--game-score", "'" + score + "'");
...
function checkHighScore() {
    if (localStorage.getItem("ice-breaker-score")) {
        if (score > localStorage.getItem("ice-breaker-score")) {
            localStorage.setItem("ice-breaker-score", score);
            document.querySelector("#start_message p span").innerHTML =
                localStorage.getItem("ice-breaker-score");
        }
        document.querySelector("#start_message p span").innerHTML =
            localStorage.getItem("ice-breaker-score");
    } else {
        localStorage.setItem("ice-breaker-score", 0);
    }
}
checkHighScore();
...
function snow() {
    ...
    b.style.opacity = ".5";
    b.style.filter = Math.random() < 0.5 ? "blur(3px)" : "blur(1px)";
    b.style.transition =
        Math.random < 0.5 ? snow_fall_rate * 0.75 + "s" : snow_fall_rate + "s";
